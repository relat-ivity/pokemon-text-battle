...
- 🎮 **完整的第九代对战系统**：支持太晶化、可配置队伍
  - **单打格式**：gen9randombattle、gen9ou
  - **双打格式**：gen9vgc2025regg（VGC）、gen9doublesou、gen9randomdoublesbattle
- 🤖 **多种AI对手**：
...
**单打格式：**
- `gen9randombattle` - 随机对战
- `gen9ou` - Over Used

**双打格式：**
- `gen9vgc2025regg` - VGC 2025 Reg G（推荐，**4选4**）
- `gen9doublesou` - Doubles OU（6v6）
- `gen9randomdoublesbattle` - 随机双打（6v6）

**注意：** VGC 格式需要从6只宝可梦中选择4只参战，其他双打格式是6v6全队参战。
...
**队伍选择：**
- **VGC 格式**（gen9vgc2025regg）：从6只宝可梦中选择4只参战
  - 输入示例：`1234`（选择1、2、3、4号宝可梦，前两个是首发）
- **其他双打格式**（gen9doublesou, gen9randomdoublesbattle）：6v6全队参战
  - 输入示例：`12`（选择1、2号宝可梦作为首发）

**战斗指令：**

双打需要为两个位置同时下达指令，指令之间用逗号分隔：

```bash
m1 +1,m2 +2           # 己方-1使用招式1攻击对手+1，己方-2使用招式2攻击对手+2
move 1 +1,switch 3    # 己方-1使用招式1攻击对手+1，己方-2切换到3号宝可梦
m1 -2,m2 +1           # 己方-1使用招式1攻击己方-2（治疗/增益），己方-2攻击对手+1
m1 +1 t,m2 +2         # 己方-1太晶化攻击对手+1，己方-2攻击对手+2
m1 1,m2 2             # 兼容简写：1和2自动转换为+1和+2（对手位置）
```
